...
class MatchContextProperty(object):
    def __init__(self, final_data_subset, to_result_data, similarity_string_threshold, similarity_quantity_threshold,
        ...
        self.final_data_subset = final_data_subset
        ...
        self.to_result_data = to_result_data
        self.equal_matched_properties = {}
        if not use_cpus:
    ...
    def calculate_property_value(self):
        ...
        if self.save_property_scores is not None:
            if os.path.isdir(self.save_property_scores):
                path_to_save = os.path.join(self.save_property_scores, 'column_' + value_of_column + '.csv')
            else:
                filename = self.save_property_scores.split('.csv')[0]
                path_to_save = filename + 'column_' + value_of_column + '.csv'
            self.properties_with_score_metric.to_csv(path_to_save, index = False)
