...
class MatchContext(object):
    def __init__(self, input_path, similarity_string_threshold, similarity_quantity_threshold,
                 string_separator, missing_property_replacement_factor, pseudo_score_gt_column_name, output_column_name,
                 save_property_scores_path, context_path=None,
                 custom_context_path=None, use_cpus=None):
        ...
        if pseudo_score_gt_column_name in self.final_data.columns:
            self.final_data[pseudo_score_gt_column_name] = self.final_data[pseudo_score_gt_column_name].astype('float')
            self.final_data_subset = self.final_data[self.final_data[pseudo_score_gt_column_name] == 1]
            self.to_result_data = self.final_data[self.final_data[pseudo_score_gt_column_name] == 0]
        else:
        ...
        self.save_property_scores_path = save_property_scores_path
    ...
    def call_for_context(self):
        obj_val = context_property_match.MatchContextProperty(self.final_data_subset, self.to_result_data,
                                                              ...
                                                              self.use_cpus,
                                                              save_property_scores=self.save_property_scores_path)
