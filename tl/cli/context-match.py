...
def parser():
    return {
        'help': 'The context-match function adds a feature column by matching the context values of each candidate to its properties and calculating the score based on the match.'
    }
...
def add_arguments(parser):
    ...
    parser.add_argument('--similarity-quantity-threshold', action='store', type=float, dest='similarity_quantity_threshold', default=0.75,
...
def run(**kwargs):
    try:
        from tl.features.context_match import Match_context
        input_file_path = kwargs.pop("input_file")
        context_file_path = kwargs.pop("context_file")
        obj = Match_context(input_file_path, context_file_path, kwargs)
